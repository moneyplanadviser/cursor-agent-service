// BUYMA 画像取り込み — 収集スクリプト【自分のDrive版（②不特定多数向け / executeAs USER_ACCESSING）】
// ローダー(image_ingest_user.gs の ?asset=js)が window.__BM = { exec, own:1 } を設定してから読み込む。
// bot版 collector.js とは別ファイル。違いは「保存先＝買い手自身のDrive自動作成（URL/共有不要）」のみ。
//   → 保存先フォルダURLの入力を撤去し、payload.dest='' で送る（GASが買い手DriveにBUYMA画像取込/{ref}を自動作成）。
// 共通ブロック(SHIRO.gate等)・収集ロジック・WebP→JPG変換は collector.js と同じ。改修時は両方に反映すること。
// 更新はこのファイルを Pages リポジトリへ push するだけ。
(function () {
  var BM = window.__BM || {};
  ...
  function picker(RULES) {
    ...
    autoScrollThen(function () {
      ...
      var root = panel(
        ...
        + '<span class="__bm_dest" data-v="drive" style="display:inline-block;padding:3px 10px;border-radius:14px;cursor:pointer;margin-right:4px;background:#1a73e8;color:#fff">自分のGoogle Drive</span>'
        ...
        + '<div id="__bm_destrow" style="margin-top:6px;color:#666;font-size:13px">保存先：あなたのGoogle Driveの「BUYMA画像取込」フォルダに自動保存されます（共有・URL入力は不要）。'
        + '<input id="__bm_destfolder" type="hidden"></div></div>'
      ...
      root.querySelector('#__bm_send').onclick = function () {
        ...
        var destUrl = ''; // 自分のDrive版：保存先URL不要（GASが買い手自身のDriveに自動作成）
        var selSet = {};
